...
public class AsyncPresentationReconciler implements IPresentationReconciler, IPresentationReconcilerExtension {
	 ...
	 */
	public AsyncPresentationReconciler() {
		super();
	 ...
	 * @param document the document whose presentation must be repaired
	 */
	private void processDamage(final IRegion damage, final IDocument document) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				if (damage != null && damage.getLength() > 0) {
					final TextPresentation p = createPresentation(damage, document);
